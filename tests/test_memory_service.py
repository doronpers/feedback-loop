...
class TestFeedbackLoopMemory:
    ...
    async def test_retrieve_patterns_rag(self, mock_memu):
        ...
        mock_memory_instance = Mock()
        mock_memory_instance.retrieve_rag = AsyncMock(return_value={
            "results": [
                {
    ...
    async def test_retrieve_patterns_llm(self, mock_memu):
        ...
        mock_memory_instance = Mock()
        mock_memory_instance.retrieve_llm = AsyncMock(return_value={
            "results": []
        })
    ...
    async def test_get_memory_stats(self, mock_memu):
        ...
        mock_memory_instance = Mock()
        mock_memory_instance.get_stats = AsyncMock(return_value={
            "total_count": 10,
            "type_counts": {
                "pattern": 5,
...
class TestPatternManagerMemoryIntegration:
    ...
    async def test_retrieve_similar_patterns_fallback(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            ...
            json.dump({
                "patterns": [
                    {"name": "test_pattern", "description": "A test pattern for testing"},
